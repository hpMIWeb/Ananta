...
const SearchFilterBar = ({ searchValue, setSearchValue, sortState = {}, setSortState = ()=>{},  showAddOn = false, defaultSortLabel, initialAddOnsValue = "All Addons" }) => {
  ...
  const onSortClick = (type) => {
    ...
    setSortState({ type, sortOrder: direction });
  };
    ...
    <>
      ...
      >
        <div className="me-auto">
          <Input
            className={classNames("headerSearchInput", "searchFilterBarInput")}
            placeholder="Search"
            value={searchValue}
            onChange={(e) => setSearchValue(e.target.value)}
            prefix={<Icon width={13.33} height={13.33} name="search" />}
          />
        </div>
        ...
        {showAddOn && (
          <div className="ms-auto">
            <Select
              ...
              clearIcon={<Icon name="downArrow" width={9} height={14} />}
              suffixIcon={<Icon name="downArrow" width={9} height={14} />}
              value={sortState?.addOnValue}
              onChange={(value) =>{
                setSortState((prev) =>({...prev, addOnValue : value}))
              }}
              allowClear
              options={[
                { value: initialAddOnsValue, label: initialAddOnsValue },
                ...
                { value: "No. Of Client Login", label: "No. Of Client Login" },
                { value: "No. Of Transactions", label: "No. Of Transactions" },
        ...
        <div className={styles.filterBoxWrapper}>
          ...
          >
            {Object.keys(sortLabel).map((label) => (
              ...
              >
                <div className={classNames("col", styles.filterBoxColumn)}>
                  <span className="d-flex">
                    <h6 className={styles.filterHeader}>{label}</h6>
                    {sortState.type === label && sortState.sortOrder && (
                      <span className={styles.filterSortLabel}>
                        ({sortState.sortOrder})
                      </span>
                    )}
                  </span>
                  <Icon
                    className={classNames(styles.filterBoxColumnIcon, {
                      [styles.alwaysShowIcon]:
                        sortState.type === label && !!sortState.sortOrder,
                      [styles.rotateArrow]:
                        sortState.type === label &&
                        sortState.sortOrder === label,
                    })}
                    name={
                      sortState.type === label
                        ? sortState.sortOrder
