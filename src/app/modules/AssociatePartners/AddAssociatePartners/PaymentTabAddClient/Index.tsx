...
const PaymentTabAddClient = ({
    ...
    const [billingMethod, setBillingMethod] = useState<string>(
        associatePartnerValue?.subscriptionDetails?.subscriptionType ||
            "subscription"
    );
    ...
    useEffect(() => {
        if (
            ...
            form.setFieldsValue({
                paymentTerms:
                    selectedAssociatePartnerData?.paymentDetails.paymentTerms,
                paymentMode:
                    selectedAssociatePartnerData?.paymentDetails.paymentMode,
                creditPeriodTime:
                    selectedAssociatePartnerData?.paymentDetails.creditPeriod,
                creditType:
                    selectedAssociatePartnerData?.paymentDetails.creditType,
            });
            setPaymentForm({
                paymentTerms:
                    selectedAssociatePartnerData?.paymentDetails.paymentTerms,
            ...
            setPaymentRowData(
                selectedAssociatePartnerData?.paymentDetails.instrumentDetails
            );
        }
    }, []);
    ...
    const onFinish = (values: any) => {
        if (billingMethod === "subscription") {
            const finalFormValues = {
                invoiceAmount:
                    associatePartnerValue?.subscriptionDetails?.invoicePrice,
            ...
            onChange(5, { paymentDetails: finalFormValues });
        } else {
            const finalFormValues = {
                instrumentDetails: [],
            };
    ...
    if (
        associatePartnerValue &&
        associatePartnerValue.subscriptionDetails &&
        associatePartnerValue.subscriptionDetails.startDate
    ) {
        futureDate = associatePartnerValue.subscriptionDetails.startDate
                        ...
                        >
                            ...
                            >
                                <p className={styles.invoiceAmount}>
                                    ...
                                    associatePartnerValue.subscriptionDetails
                                        ? associatePartnerValue
                                              .subscriptionDetails.invoicePrice
                                        : 0}
                                    /-
                                </p>
                            </div>
