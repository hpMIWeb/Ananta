...
const SubComplianceViewEdit = (props: any) => {
  ...
  const editCommentHandler = (
    ...
    api
      .updateSubComplianceComment(updateComment)
      .then((resp: any) => {
  ...
  const deleteCommentHandler = (commentId: string, parentId: string) => {
    api
      .deleteComplianceComment(props.complianceId, commentId, subCompliance._id)
      .then((resp: any) => {
        toast.success("Successfully deleted comment", {
          position: toast.POSITION.TOP_RIGHT,
        });
        const newSubComments = subComments.filter((comment: IComment) => {
      ...
      <div key={subCompliance._id}>
        ...
        <div className="sub-compliance-content">
          <Row gutter={[8, 8]} className="form-row">
            ...
            >
              <div className="timerbuttons">
                {subComplianceClients && subComplianceClients.length <= 0 && (
                  <Stopwatch
                    ...
                    handleTaskStatus={(
                      ...
                      handleTaskStatus(isRunning, time, isStop, subCompliance);
                    }}
                ...
                {subComplianceClients && subComplianceClients.length > 0 && (
                  <span className="stopwatch-time">
                    {getTotalTime(
                      subComplianceClients.map((item: IClientDetails) => {
            ...
            >
              {!props.isEdit && (
                <Title
                  ...
                  className={`text-priority ${subCompliance.priority} ${
                    subCompliance.priority === "high" ? "blink" : ""
                  }`}
                >
              ...
              {props.isEdit && (
                <Select
                  ...
                  onChange={(value, event) => {
                    statusChangeHandler(event, value, subCompliance);
                  }}
                />
              )}
            </Col>
            ...
            <Col xs={{ span: 24 }} sm={{ span: 24 }} md={{ span: 24 }}>
              ...
              {props.isEdit && (
                <Input
                  ...
                  onChange={(event) => {
                    inputChangeHandler(event, subCompliance);
                  }}
                />
              )}
            </Col>
          ...
          <Row gutter={[8, 8]} className="form-row">
            <Col xs={{ span: 24 }} sm={{ span: 24 }} md={{ span: 24 }}>
              {!props.isEdit && (
                <b>{subCompliance.remark && parse(subCompliance.remark)}</b>
              )}
              {props.isEdit && (
                <ReactQuill
                  theme="snow"
                  ...
                  placeholder="Remark"
                  onChange={(event) => {
                    inputChangeHandler(event, subCompliance, "remark");
                  }}
                />
              )}
            </Col>
