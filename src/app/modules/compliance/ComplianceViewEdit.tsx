...
const ComplianceViewEdit = (props: any) => {
  ...
  const [complianceClients, setComplianceClients] = useState<IClientDetails[]>(
  ...
  const handleTaskStatus = (
    ...
    complianceUpdate.status = isStop ? Status.completed : Status.in_progress;
    if (!isRunning) complianceUpdate.actual_time = time;

    api
      .updateCompliance(updateCompliance._id, complianceUpdate)
      .then((resp: any) => {
  ...
  const handleClientStatus = (
    ...
    let matchedItem = complianceClients.find((clientItem: IClientDetails) => {
  ...
  const deleteCommentHandler = (commentId: string, parentId: string) => {
    api
      .deleteComplianceComment(updateCompliance._id, commentId, "")
      .then((resp: any) => {
        toast.success("Successfully deleted comment", {
          position: toast.POSITION.TOP_RIGHT,
        });

        const newSubComments = complianceComments.filter((comment: any) => {
  ...
  const handleUpdateTask = () => {
    ...
    api
      .updateCompliance(updateCompliance._id, updateCompliance)
      .then((resp: any) => {
        toast.success("Successfully Updated Compliance.", {
          position: toast.POSITION.TOP_RIGHT,
        });
  ...
  const subComplianceHeader = (subComplianceItem: any, index: number) => {
    return (
      <div className="sub-task-header">
        ...
        {props.fullScreenMode && index.toString() !== currentCollapse && (
          <div
            className="task-header-cell"
            style={{
        ...
        {(index.toString() !== currentCollapse || !props.fullScreenMode) && (
          <div
            className="task-header-cell"
            style={{
              flex: props.fullScreenMode ? 0.2 : 2,
          ...
          >
            ...
            />
            {subComplianceItem.comments ? subComplianceItem.comments.length : 0}
          </div>
        )}
        {index.toString() !== currentCollapse && props.fullScreenMode && (
          <div
        ...
        >
          {props.fullScreenMode ? capitalize(subComplianceItem.priority) : " "}
        </div>
      </div>
    );
  };
  ...
  return (
    <>
      <div
        style={{
          display: Object.keys(updateCompliance).length > 0 ? "block" : "none",
        }}
      >
        <ToastContainer />
        <Form style={{ padding: "15px 0 0 0" }}>
          <Row gutter={[8, 8]} className="form-row">
            ...
            >
              <FontAwesomeIcon
                icon={
                  props.fullScreenMode ? faCompressArrowsAlt : faExpandArrowsAlt
                }
          ...
          <Row gutter={[8, 8]} className="form-row">
            ...
            >
              <div className="timerbuttons">
                {subCompliances && subCompliances.length <= 0 && (
                  <Stopwatch
            ...
            >
              {!isEdit && (
                <Title
                  ...
                  className={`text-priority ${updateCompliance.priority} ${
                    updateCompliance.priority === "high" ? "blink" : ""
                  }`}
                >
                ...
                {updateCompliance.subcompliance.map(
                  (subComplianceItem: ISubCompliance, index: number) => (
                    <CollapsePanel
                      header={
                        !isEdit
                          ? subComplianceHeader(subComplianceItem, index)
                          : subComplianceItem.title
                      }
                      key={index}
                    >
                      <SubComplianceViewEdit
                        ...
                        isEdit={isEdit}
                        complianceId={updateCompliance._id}
                        handleListUpdate={updateSubComplianceList}
                        handleComplianceUpdate={updateCurrentCompliance}
                      />
                    </CollapsePanel>
                  )
                )}
