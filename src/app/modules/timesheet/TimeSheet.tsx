...
const TimeSheet = () => {
    ...
    const [addTimesheet, setAddTimesheet] = useState<ITimesheet>(
        new Timesheet()
    );
    ...
    const addNewTimesheetRow = () => {
        ...
        const newTimesheet = {
            _id:
                allTimesheet && allTimesheet.length > 0
                    ? allTimesheet.length + 1
                    : 1,
            start_time: "",
            end_time: "",
            remark: "",
    ...
    const editClickHandler = (updateTimesheet: Timesheet) => {
        ...
        api.updateTimesheet(updateTimesheet._id, updateTimesheet).then(
            (resp: any) => {
    ...
    const addTimeSheetHandler = () => {
        ...
        const overlappingRecord = allTimesheet.find((item: Timesheet) => {
            ...
            return (
                (startTime.isSame(itemEndTime) &&
                    endTime.isAfter(itemStartTime)) ||
                (startTime.isAfter(itemStartTime) &&
                    endTime.isSame(itemEndTime)) ||
                (startTime.isBefore(itemStartTime) &&
                    endTime.isAfter(itemEndTime))
            );
        });
