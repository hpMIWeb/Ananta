...
const AddEmployee = () => {
  ...
  const getEmployeesListSuccess = useSelector(
    (state) => state.getEmployees.success
  );
  const getEmployeesListLoading = useSelector(
    (state) => state.getEmployees.loading
  );
  const getEmployeesList = useSelector((state) => state.getEmployees.data);
  
  const { loading, success } = useSelector((state) => state.createEmployee);
  ...
  const onChange = (key, formInfo) => {
    if(key === 6){
      const payload = {...employeeDetails, ...formInfo}
      dispatch(createEmployeeReducersApi({payload : payload}));
    }else{
      setActiveTab(key);
    }
  };
  ...
  const items = [
    {
      key: 1,
      label: `Basic Info`,
      children: <BasicInfo onChange={onChange} setEmployeeInfo={setEmployeeInfo} />,
    },
    {
      key: 2,
      label: `Organisation Details`,
      children: <OrganisationDetails onChange={onChange} setEmployeeInfo={setEmployeeInfo} />,
    },
    {
      key: 3,
      label: `Assign Clients`,
      children: <AssignClient onChange={onChange} setEmployeeInfo={setEmployeeInfo} />,
    },
    {
      key: 4,
      label: `Bank Details`,
      children: <BankDetails onChange={onChange} setEmployeeInfo={setEmployeeInfo} />,
    },
    {
      key: 5,
      label: `Emergency Details`,
      children: <OwnerInfo onChange={onChange} loading={loading} setEmployeeInfo={setEmployeeInfo} />,
    }
  ];
