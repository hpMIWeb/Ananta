import { useSelector } from "react-redux";
import { DatePicker, Form, InputNumber, TimePicker } from "antd";
import styles from "./organisationDetails.module.scss";
import CardBottomAction from "../EmergencyInfo/CardBottomAction";
import { useState } from "react";
import classNames from "classnames";
import Input from "../../../../../components/Input/Index";
import Select from "../../../../../components/Select/Index";

const initialBranchInfoData = [{ type: "default", index: 0 }];

const OrganisationDetails = ({ onChange, setEmployeeInfo }) => {
  const getRolesList = useSelector((state) => state.getRoles.data) || [];
  const getRoleTypesList = useSelector((state) => state.getRoleType.data) || [];
  const getDepartmentsList =
    useSelector((state) => state.getDepartments.data) || [];
  const [branchInfoData, setBranchInfoData] = useState([]);
  const [branchChecked, setBranchChecked] = useState(false);
  console.log("getDepartmentsList", getDepartmentsList);
  const onFinish = (value) => {
    setEmployeeInfo(value);
    onChange(3);
  };

  const addMoreOwnerCard = () => {
    setBranchInfoData((prev) => [
      ...prev,
      { type: "new", index: branchInfoData.length },
    ]);
  };

  const onDeleteCardClick = (cardIndex) => {
    const newOwnerInfoData = branchInfoData.filter(
      (a) => a.index !== cardIndex
    );
    setBranchInfoData(newOwnerInfoData);
  };

  const handleBranchChecked = (cheked) => {
    setBranchChecked(cheked);
    if (cheked) {
      setBranchInfoData(initialBranchInfoData);
    } else {
      setBranchInfoData([]);
    }
  };

  return (
    <div>
      <Form
        name="basic"
        initialValues={{ remember: true }}
        onFinish={onFinish}
        autoComplete="off"
        requiredMark={false}
        className="customAddForm"
      >
        <div className="col-12 col-md-12 col-lg-12">
          <div className={styles.branchFieldWrapper}>
            <div className={classNames("row", styles.formFieldWrapper)}>
              <div
                className={classNames(
                  "col-12 col-md-3 col-lg-3",
                  styles.fieldPadding8
                )}
              >
                <div className="mb-3">
                  <label
                    style={{ marginBottom: 7 }}
                    className="custom-label"
                    for=""
                  >
                    Role<sup class="text-danger fs--1">*</sup>
                  </label>
                  <Form.Item
                    name="role"
                    className="customAddFormSelectOptions"
                    rules={[
                      {
                        required: true,
                        message: "Please Select your Role!",
                      },
                    ]}
                  >
                    <Select
                      placeholder="Select Role"
                      options={getRolesList.map((role) => ({
                        label: role.roleName,
                        value: role._id,
                      }))}
                    />
                  </Form.Item>
                </div>
              </div>
              <div
                className={classNames(
                  "col-12 col-md-3 col-lg-3",
                  styles.fieldPadding8
                )}
              >
                <div className="mb-3">
                  <label
                    style={{ marginBottom: 7 }}
                    className="custom-label"
                    for=""
                  >
                    Designation
                  </label>
                  <Form.Item
                    name="designation"
                    className="customAddFormSelectOptions"
                  >
                    <Select
                      placeholder="Select Designation"
                      options={getRoleTypesList.map((role) => ({
                        label: role?.role_type,
                        value: role?._id,
                      }))}
                    />
                  </Form.Item>
                </div>
              </div>
              <div
                className={classNames(
                  "col-12 col-md-3 col-lg-3",
                  styles.fieldPadding8
                )}
              >
                <div className="mb-3">
                  <label
                    style={{ marginBottom: 7 }}
                    className="custom-label"
                    for=""
                  >
                    Team
                  </label>
                  <Form.Item name="team" className="customAddFormSelectOptions">
                    <Select
                      placeholder="Select Team"
                      options={[
                        { label: "Team 1", value: "team1" },
                        { label: "Team 2", value: "team2" },
                        { label: "Team 3", value: "team3" },
                      ]}
                    />
                  </Form.Item>
                </div>
              </div>
              <div
                className={classNames(
                  "col-12 col-md-3 col-lg-3",
                  styles.fieldPadding8
                )}
              >
                <div className="mb-3">
                  <label
                    style={{ marginBottom: 7 }}
                    className="custom-label"
                    for=""
                  >
                    Department
                  </label>
                  <Form.Item
                    name="department"
                    className="customAddFormSelectOptions"
                  >
                    <Select
                      placeholder="Select Department"
                      options={getDepartmentsList.map((role) => ({
                        label: role?.department_name,
                        value: role?._id,
                      }))}
                    />
                  </Form.Item>
                </div>
              </div>
              <div
                className={classNames(
                  "col-12 col-md-3 col-lg-3",
                  styles.fieldPadding8
                )}
              >
                <div className="mb-3">
                  <label
                    style={{ marginBottom: 7 }}
                    className="custom-label"
                    for=""
                  >
                    Aadhar
                  </label>
                  <Form.Item
                    name="aadhar"
                    className="customAddFormSelectOptions"
                    rules={[
                      {
                        len: 12,
                        message: "Aadhar number should be exactly 12 digits!",
                      },
                    ]}
                  >
                    <InputNumber
                      style={{ width: "100%" }}
                      className="customInputNumber"
                      placeholder="Aadhar Number"
                      min={0}
                    />
                  </Form.Item>
                </div>
              </div>
              <div
                className={classNames(
                  "col-12 col-md-3 col-lg-3",
                  styles.fieldPadding8
                )}
              >
                <div className="mb-3">
                  <label
                    style={{ marginBottom: 7 }}
                    className="custom-label"
                    for=""
                  >
                    PAN Number<sup class="text-danger fs--1">*</sup>
                  </label>
                  <Form.Item
                    name="pan"
                    className="customAddFormSelectOptions"
                    rules={[
                      {
                        required: true,
                        message: "Please Enter your PAN Number!",
                      },
                      {
                        pattern: /^[A-Z]{5}[0-9]{4}[A-Z]{1}$/,
                        message: "Invalid PAN Number! Format: ABCDE1234F",
                      },
                    ]}
                  >
                    <Input
                      placeholder="PAN Number"
                      className="customAddFormInputText"
                    />
                  </Form.Item>
                </div>
              </div>
              <div
                className={classNames(
                  "col-12 col-md-3 col-lg-3",
                  styles.fieldPadding8
                )}
              >
                <div className="mb-3">
                  <label
                    style={{ marginBottom: 7 }}
                    className="custom-label"
                    for=""
                  >
                    Date of Joining
                  </label>
                  <Form.Item
                    name="dateOfJoining"
                    className="customAddFormSelectOptions"
                  >
                    <DatePicker
                      placeholder="Date of Joining"
                      className="customFormDatePicker"
                      format="DD/MM/YYYY"
                    />
                  </Form.Item>
                </div>
              </div>
              <div
                className={classNames(
                  "col-12 col-md-3 col-lg-3",
                  styles.fieldPadding8
                )}
              >
                <div className="mb-3">
                  <label
                    style={{ marginBottom: 7 }}
                    className="custom-label"
                    for=""
                  >
                    Last Date in Firm
                  </label>
                  <Form.Item
                    name="lastDayInFirm"
                    className="customAddFormSelectOptions"
                  >
                    <DatePicker
                      placeholder="Last Date in Firm"
                      className="customFormDatePicker"
                      format="DD/MM/YYYY"
                    />
                  </Form.Item>
                </div>
              </div>
              <div
                className={classNames(
                  "col-12 col-md-3 col-lg-3",
                  styles.fieldPadding8
                )}
              >
                <div className="mb-3">
                  <label
                    style={{ marginBottom: 7 }}
                    className="custom-label"
                    for=""
                  >
                    Working Time (From)
                  </label>
                  <Form.Item
                    name="workingTimeFrom"
                    className="customAddFormSelectOptions"
                  >
                    <TimePicker
                      placeholder="From"
                      className="customFormDatePicker"
                      use12Hours
                      format="h:mm a"
                    />
                  </Form.Item>
                </div>
              </div>
              <div
                className={classNames(
                  "col-12 col-md-3 col-lg-3",
                  styles.fieldPadding8
                )}
              >
                <div className="mb-3">
                  <label
                    style={{ marginBottom: 7 }}
                    className="custom-label"
                    for=""
                  >
                    Working Time (To)
                  </label>
                  <Form.Item
                    name="workingTimeTo"
                    className="customAddFormSelectOptions"
                  >
                    <TimePicker
                      placeholder="To"
                      className="customFormDatePicker"
                      format="h:mm a"
                      use12Hours
                    />
                  </Form.Item>
                </div>
              </div>
              <div
                className={classNames(
                  "col-12 col-md-3 col-lg-3",
                  styles.fieldPadding8
                )}
              >
                <div className="mb-3">
                  <label
                    style={{ marginBottom: 7 }}
                    className="custom-label"
                    for=""
                  >
                    Allow Login?
                  </label>
                  <Form.Item
                    name="allowLogin"
                    className="customAddFormSelectOptions"
                  >
                    <Select
                      placeholder="Select Type"
                      options={[
                        { value: "Yes", label: "Yes" },
                        { value: "No", label: "No" },
                      ]}
                    />
                  </Form.Item>
                </div>
              </div>
              <div
                className={classNames(
                  "col-12 col-md-3 col-lg-3",
                  styles.fieldPadding8
                )}
              >
                <div className="mb-3">
                  <label
                    style={{ marginBottom: 7 }}
                    className="custom-label"
                    for=""
                  >
                    Reference
                    <sup class="text-danger fs--1">*</sup>
                  </label>
                  <Form.Item
                    name="referance"
                    className="customAddFormSelectOptions"
                    rules={[
                      {
                        pattern: /^[a-zA-Z0-9]*$/,
                        message: "Only alphanumeric characters are allowed.",
                      },
                    ]}
                  >
                    <Input
                      placeholder="Reference"
                      className="customAddFormInputText"
                    />
                  </Form.Item>
                </div>
              </div>
            </div>
          </div>
        </div>
        <CardBottomAction addCardClick={addMoreOwnerCard} onChange={onChange} />
      </Form>
    </div>
  );
};

export default OrganisationDetails;
