...
const TaskViewEdit = (props: any) => {
  ...
  const [updateTask, setUpdateTask] = useState<AddTask>(props.tableRowSelected);
  ...
  const updateCurrentTask = (subTask: SubTask) => {
    if (subTask && taskSubTasks) {
      const newData = taskSubTasks.map((subTaskItem: SubTask) =>
        subTaskItem && subTaskItem._id === subTask._id ? subTask : subTaskItem
      );
  ...
  const editCommentHandler = (
    ...
    api
      .updateTaskComment(updateComment)
      .then((resp: any) => {
  ...
  const deleteCommentHandler = (commentId: string, parentId: string) => {
    api
      .deleteTaskComment(updateTask._id, commentId)
      .then((resp: any) => {
    ...
    return (
      <div className="sub-task-header">
        ...
        {index.toString() !== currentCollapse && (
          ...
          >
            {props.fullScreenMode ? capitalize(subTaskItem.priority) : " "}
          </div>
        )}
      </div>
    );
            ...
            {!isEdit && (
              <>
                <Title
                  ...
                  className={`text-priority ${updateTask.priority} ${
                    updateTask.priority === "high" ? "blink" : ""
                  }`}
                >
            ...
            <div>
              {!isEdit && (
                <div className="assigneeContainer">
                  ...
                  >
                    {updateTask.assigned_to.map((assignee: any, index: any) => (
            ...
            <div>
              {!isEdit && (
                <b>
                  ...
                  />
                  {dayjs(updateTask.start_date).format("YYYY-MM-DD, HH:mm A")}
                </b>
              )}
            ...
            <div>
              {!isEdit && (
                <b>
                  ...
                  />
                  {dayjs(updateTask.due_date).format("YYYY-MM-DD, HH:mm A")}
                </b>
              )}
              {isEdit && (
                <DatePicker
          ...
          {!props.fullScreenMode && (
            ...
            >
              ...
              <div>
                {
                  <b>
                    {updateTask.client && updateTask.client.length > 0
        ...
        <Row gutter={[8, 8]} className="form-row">
          <Col xs={{ span: 24 }} sm={{ span: 24 }} md={{ span: 24 }}>
            ...
            {taskSubTasks.length > 0 && (
              ...
              >
                {taskSubTasks.map((subTaskItem: SubTask, index: number) => {
                  return (
                    <CollapsePanel
                      header={
                        !isEdit
                          ? subTaskHeader(subTaskItem, index)
                          : subTaskItem.title
                      }
                      key={index}
                    >
                      <SubTaskViewEdit
                        ...
                        isEdit={isEdit}
                        parentId={updateTask._id}
                        handleListUpdate={updateTaskList}
                        handleTaskUpdate={updateCurrentTask}
                      />
                    </CollapsePanel>
                  );
                })}
