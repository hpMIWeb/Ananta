...
const SubTaskViewEdit = (props: any) => {
  ...
  const handleUpdateTask = () => {
    api.updateSubTask(updateSubTask._id, updateSubTask).then((resp: any) => {
  ...
  const addCommentHandler = (comment: string) => {
    ...
    api
      ...
      .then((resp: any) => {
        ...
        if (props.handleListUpdate) props.handleListUpdate(respData.subtask);
      })
      .catch((error: any) => {
  ...
  const deleteCommentHandler = (commentId: string, parentId: string) => {
    api
      .deleteTaskComment(updateSubTask._id, commentId)
      .then((resp: any) => {
  ...
  return (
    <>
      <div
        style={{
          display: Object.keys(updateSubTask).length > 0 ? "block" : "none",
        }}
      >
        <ToastContainer />
        <Row gutter={[8, 8]} className="form-row">
          ...
          >
            {!isEdit && (
              <Title
                ...
                className={`text-priority ${updateSubTask.priority} ${
                  updateSubTask.priority === "high" ? "blink" : ""
                }`}
              >
