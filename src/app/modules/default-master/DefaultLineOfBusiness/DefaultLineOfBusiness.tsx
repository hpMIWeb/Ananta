...
import styles from "./defaultLineOfBusiness.module.scss";
...
const DefaultLineOfBusiness = () => {
  ...
  const deleteClickHandler = (industryTypeId: string) => {
    ...
    setLoading(true); // Set loading state to true
    api
      .deleteDefaultLineOfBusiness(industryTypeId)
      .then((resp: any) => {
        const updatedData = lineOfBusinessList.filter(
          (item: IDefaultLineOfBusiness) => item._id !== industryTypeId
        );
      ...
      .catch((error) => {
        toast.error("Technical error while deleting Line Of Business.", {
          position: toast.POSITION.TOP_RIGHT,
        });
      });
  };
    ...
    form
      ...
      .then((values) => {
        try {
          ...
          const apiCall =
            modalMode === "add"
              ? api.createMultipleDefaultLineOfBusiness(selectedBusinessTypes)
          ...
          apiCall
            .then((resp: any) => {
              ...
              form.setFieldsValue({} as IAddDefaultLineOfBusiness);
          ...
          toast.error("Technical error while creating Line Of Business.", {
            position: toast.POSITION.TOP_RIGHT,
          });
        }
      })
      ...
      <Modal
        title={
          modalMode === "add"
            ...
            : "Edit " + selectedLineOfBusiness.name + " Line Of Business"
        }
        open={isModalOpen}
        onOk={handleOk}
          ...
          {selectedBusinessTypes.map((businessName, index) => (
            <Tag
              ...
              onClose={() => {
                ...
                const updatedLineOfBusinessNames = [...selectedBusinessTypes];
                updatedLineOfBusinessNames.splice(index, 1);
                setSelectedBusinessTypes(updatedLineOfBusinessNames);
              }}
              style={{ marginTop: "8px" }}
            >
              {businessName.name}
