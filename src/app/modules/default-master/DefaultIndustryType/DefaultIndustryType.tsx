...
const DefaultIndustryType = () => {
  ...
  const deleteClickHandler = (industryTypeId: string) => {
    ...
    setLoading(true); // Set loading state to true
    api
      .deleteDefaultIndustryType(industryTypeId)
      .then((resp: any) => {
    ...
    form
      ...
      .then((values) => {
        ...
        try {
          ...
          const apiCall =
            modalMode === "add"
              ? api.createMultipleDefaultIndustryType(selectedIndustryNames)
          ...
          toast.error("Technical error while creating Industry Type.", {
            position: toast.POSITION.TOP_RIGHT,
          });
        }
      })
            ...
            {selectedIndustryNames.map((industry, index) => (
              <Tag
                ...
                onClose={() => {
                  ...
                  const updatedIndustryNames = [...selectedIndustryNames];
