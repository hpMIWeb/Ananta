    ...
    <div>
      <div className="d-flex w-100">
        <div className="me-auto">
          <div className="d-flex">
            <div className="flex-shrink-0">
              <span className={styles.cardPointCounter}>{displayIndex}</span>
            </div>
            <div className="flex-grow-1 ms-2">
              <h5
                className={classNames("mb-0", styles.subscriptionNameLabel, {
                  [styles.subscriptionNameLabelBlue]: planNameLabelBlue,
                })}
              >
                <span
                ...
                {planNameLabel && (
                  <span
                    className={styles.subscriptionNameLabelDesc}
                  >{`( ${planNameLabel} )`}</span>
                )}
              </h5>
              {titleDesc && <p className={styles.cardTitleDesc}>{titleDesc}</p>}
            </div>
          </div>
        </div>
        <div className="ms-auto">
          {isProfileViewAction ? (
            <div className="d-flex">
              <button className="transparentBtn" onClick={profileClick}>
                <Icon name="profile" />
                Profile
              </button>
              <div className="d-flex align-items-center">
                ...
                />
                <label className={styles.cardSwitchLabel}>Active</label>
              </div>
            </div>
          ) : (
            <div className="d-flex align-items-center">
              ...
              <span
                className={classNames(styles.switchDevider, "mx-2 text-400")}
              >
                |
              </span>
              <button
              ...
              <button
                ...
                className={styles.transparentHistoryBtn}
                onClick={() =>
                  handleSubscriptionHistoryModalClick(historyData, id)
                }
              >
                (History)
              </button>
      ...
      <div className={styles.cardDescWrapper}>
        <div
          className={classNames(
            ...
            {
              ["row row-cols-1 row-cols-sm-4 row-col-md-4"]: column === 4,
            },
            {
              ["row row-cols-1 row-cols-sm-3 row-col-md-3"]: column === 3,
            }
          )}
        >
